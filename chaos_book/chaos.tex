...
\begin{align*}
	y =& x\\
	z =& \frac{xy}{b} \\
(x,y,z)=& (0,0,0)\text{ Is one solution.} \numberthis \label{eq:lorenz:eq:eq0}
\end{align*}
The other solutions are given by substituting in \eqref{eq:lorenz:eq:2}:
\begin{align*}
...
The fixed points follow from making the substitution  \eqref{eq:miranda:stone:1} and \eqref{eq:miranda:stone:2} in \eqref{eq:lorenz:eq:eq0} and \eqref{eq:lorenz:eq:eq}. The first equation yields.
\begin{align*}
	(u,v,z)=& (0,0,0)\text{, and the second}\\
	(u,v,z)=& \big(0,2b(\rho-1),\rho-1\big) \text{. Both fixed points coincide.}
\end{align*}
\section{Homework 2}
